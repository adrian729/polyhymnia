...
import { measure, note, resetIdCounter, score, tuplet } from '../src/build/index.js';
...
import * as R from '../src/model/rational.js';
import { durationToRational, durationToTicks, isNotatable, timeCapacity } from '../src/model/duration.js';
import { DEFAULT_DIVISIONS } from '../src/model/types.js';

beforeEach(() => resetIdCounter());

describe('Rational', () => {
  it('normalizes by gcd with the sign on the numerator', () => {
    expect(R.rational(6, 8)).toEqual({ n: 3, d: 4 });
    expect(R.rational(-6, 8)).toEqual({ n: -3, d: 4 });
    expect(R.rational(6, -8)).toEqual({ n: -3, d: 4 });
    expect(R.rational(0, 5)).toEqual({ n: 0, d: 1 });
  });

  it('sums repeated fractions exactly, where binary floats accumulate error', () => {
    const third = R.rational(1, 3);
    expect(R.add(R.add(third, third), third)).toEqual({ n: 1, d: 1 });

    // A 4/4 bar of 7:4 septuplet eighths: 7 x 1/14 of a whole note.
    const septuplet = R.rational(1, 14);
  ...
  it('compares and orders without epsilon', () => {
    expect(R.compare(R.rational(1, 3), R.rational(1, 2))).toBe(-1);
    expect(R.compare(R.rational(2, 4), R.rational(1, 2))).toBe(0);
    expect(R.compare(R.rational(5, 8), R.rational(1, 2))).toBe(1);
  });
  ...
  it('is exact under add/subtract round-trips', () => {
    fc.assert(
      fc.property(
        ...
        fc.integer({ min: 1, max: 500 }),
        (an, ad, bn, bd) => {
          const a = R.rational(an, ad);
          const b = R.rational(bn, bd);
    ...
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 40 }), (sixteenths) => {
        ...
        const total = rests
          ...
          .reduce((sum, l) => R.add(sum, l), R.ZERO);
        expect(total).toEqual(R.rational(sixteenths, 16));
      }),
    );
  });
});
