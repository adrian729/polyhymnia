...
- `beams` (this pipeline's one `mnx-unsupported` allowlist entry, `check.ts`) never fired in this batch — the converter's own README says part-measure beam encoding is still "TODO" on its side, so it doesn't emit `beams` yet either way. The allowlist stays in `check.ts` for when it does (and for hand-authored/future-converter content that does emit beams); nothing to revisit here.
